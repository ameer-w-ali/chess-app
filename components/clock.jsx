...
export default function Clock({active}) {
  ...
  return (
    <div className={`w-28 rounded flex items-center text-center px-4 font-bold  ${active ? 'bg-neutral-200 text-neutral-800' : 'bg-neutral-400 text-neutral-800 opacity-85'}`}>
      <Clock12
        strokeWidth={3}
        className={`mr-3 h-5 w-5 ${!active && "invisible"}`}
        style={{ transform: `rotate(${rotationDegrees}deg)` }}
      />
      <span>{counter.minutes < 10 ? '0' : ''}{counter.minutes}:{counter.seconds < 10 ? '0' : ''}{counter.seconds}</span>
    </div>
  )
}